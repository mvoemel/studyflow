...
export async function GET(request: Request) {
  ...
  const degreeId = url.searchParams.get("degreeId") || "degree-1";
  const semesterId = url.searchParams.get("semesterId") || "semester-1";

  const filteredEvents = events.filter(
    (event) => event.degreeId === degreeId && event.semesterId === semesterId
  );
